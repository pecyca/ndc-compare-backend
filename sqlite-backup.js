...
/** Build a RAM suggest index from v_ndc_suggest. Be tolerant of missing columns. */
export async function buildSuggestIndex({ limit = 250000 } = {}) {
    ...
    // Try the richer shape first; fall back if columns are missing.
    try {
    ...
    suggestIndex = rows.map(r => ({
        ...
        substance: hasSubstance ? (r.substance || null) : null,
        strength: null,                 // keep API stable; null if not available
        ...
        _sub: hasSubstance ? String(r.substance || '').toLowerCase() : '',
    }));
